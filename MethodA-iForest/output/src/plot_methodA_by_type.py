...
Type-specific anomaly visualizations for the Method A by-type pipeline.

Each plot type matches what is being explained:
  speed    -> SOG vs time  (anomaly window in red, fleet 99th pct reference line)
  turn     -> |course change| vs time  (same)
  timegap  -> inter-ping gap vs time   (anomalous gap bar in red)
  distance -> segment distance vs time + lat/lon jump view

Reference lines are drawn on every plot using fleet-wide percentile stats
saved by score_methodA_by_type.py in reference_stats.json.
Without a reference line a viewer cannot tell if a value is unusual.

For each type: n_top highest-score + n_random random from top 1%.
Run after score_methodA_by_type.py.
"""
...
def add_ref_line(ax, value, label, color='orange', linestyle='--'):
    """Draw a horizontal reference line and label it."""
    if value is None:
        return
...
def load_events(csv_path, n_top, n_random, seed=42):
    """
    Load top and random anomaly events from a type-specific top-1% CSV.

    For the 'top' selection we first deduplicate by MMSI (keep only the
    highest-scoring segment per vessel), then take the n_top best.
    This prevents one fast vessel from occupying all top-N slots.

    For the 'random' selection we sample from the full remaining rows
    (after removing the top-N vessels) to ensure diversity.
    """
    ...
    # One row per MMSI: keep highest-scoring segment for each vessel
    dedup = df.drop_duplicates(subset=['MMSI'], keep='first').reset_index(drop=True)
    top_events = dedup.head(n_top).to_dict('records')
    for e in top_events:
        e['sample_kind'] = 'top'

    # Random pool: remove the MMSI already chosen for top plots
...
def stream_vessel_points(points_csv, mmsi_set):
    """Read ais_points_clean.csv once, collect all points for mmsi_set."""
...
def plot_speed(event, vessel_points, out_path, ref):
    """
    SOG vs time. Anomaly window in red.
    Orange dashed line = fleet 99th percentile SOG (reference for 'what is high').
    """
    ...
    # Reference lines from fleet-wide stats
    ...
    # Annotate the peak SOG inside the anomaly window
    if inside:
    ...
    ax.set_title(
        ...
        f"[score={score_val:.1f}, p{pct_val:.2f}]\n"
...
def plot_turn(event, vessel_points, out_path, ref):
    ...
    Orange dashed line = fleet 99th percentile |Δcog|.
    """
    t_prev = parse_iso(event['t_prev'])
    t_curr = parse_iso(event['t_curr'])
    if t_prev is None or t_curr is None:
    ...
    # Annotate the exact anomalous turn value
...
def plot_timegap(event, vessel_points, out_path, ref):
    """
    Two-panel figure that makes the silent period visually obvious:

    Left panel — Ping timeline:
      Each AIS ping is drawn as a short vertical tick on a time axis.
      The anomalous silent period is a large red shaded empty space.
      A double-headed arrow labels the exact gap duration.
      This makes "the vessel went silent" immediately readable.

    Right panel — Bar comparison:
      Three bars: fleet median | fleet p99 | this gap.
      Directly answers 'how abnormal is this?' without any calculation.
    """
    t_prev = parse_iso(event['t_prev'])
    t_curr = parse_iso(event['t_curr'])
    if t_prev is None or t_curr is None:
    ...
    # Collect pings in ±1 h around the gap endpoints (tight window keeps plot clean)
    ctx = get_context(vessel_points, t_prev, t_curr, before_s=3600, after_s=3600)
    ...
    # Draw each ping as a thin vertical tick at y=0
    for grp, color in [(before, 'steelblue'), (after, 'steelblue')]:
        if grp:
            ax_tl.vlines([p[0] for p in grp], -0.4, 0.4,
                         colors=color, lw=1.2, alpha=0.7)

    # Red shaded region = the silent period
    ax_tl.axvspan(t_prev, t_curr, color='red', alpha=0.18, label='silent period')

    # Double-headed arrow spanning the gap (drawn in axis-fraction y so it
    # stays above the ticks regardless of data scale)
...
def plot_distance(event, vessel_points, out_path, ref):
    """
    Two-panel figure:
      Left  — segment distance (m) vs time; anomalous jump in red.
      Right — lat/lon local view with jump arrow.
    Fleet p99 distance shown as reference line on left panel.
    """
    t_prev = parse_iso(event['t_prev'])
    t_curr = parse_iso(event['t_curr'])
    if t_prev is None or t_curr is None:
    ...
    ax_left.legend(
        handles=[Patch(color='red', label='anomaly jump'),
                 ...
                 Patch(color='orange', label=f'fleet p99 ({p99_dist/1000:.1f} km)')],
        fontsize=8
    )
    fmt_hm(ax_left)
...
def main():
    parser = argparse.ArgumentParser(
        description='Type-specific anomaly plots for Method A by-type pipeline'
    )
    parser.add_argument('--by-type-dir', default='output/methodA_by_type')
    parser.add_argument('--points', default='output/ais_points_clean.csv')
    parser.add_argument('--out-dir', default='output/figs/methodA_by_type')
    parser.add_argument('--n-top', type=int, default=5)
    parser.add_argument('--n-random', type=int, default=5)
    parser.add_argument('--context-min', type=int, default=60)
    ...
    # Load reference stats (fleet-wide percentiles for reference lines)
    ref_path = os.path.join(args.by_type_dir, 'reference_stats.json')
    if os.path.exists(ref_path):
        with open(ref_path) as f:
            ref_stats = json.load(f)
        print(f'Loaded reference stats from {ref_path}')
    else:
        print(f'WARNING: reference_stats.json not found. '
              f'Re-run score_methodA_by_type.py to generate it. '
    ...
    for type_name in PLOT_FN:
        csv_path = os.path.join(args.by_type_dir, f'{type_name}_top1pct.csv')
        if not os.path.exists(csv_path):
            print(f'[{type_name}] top-1% CSV not found, skipping: {csv_path}')
            continue
        events = load_events(csv_path, args.n_top, args.n_random, args.random_seed)
    ...
    if not all_mmsi:
        print('No events found. Run score_methodA_by_type.py first.')
        return

    # Stream AIS points once for all needed vessels
    print(f'\nStreaming points for {len(all_mmsi)} vessels from {args.points} ...')
